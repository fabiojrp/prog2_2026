...
appAdmin.post('/produtos/cadastrar', upload.single('imagem'), (req, res) => {
    ...
    const destaque = parseInt(req.body.destaque) || 0;

    db.run(
        `INSERT INTO livros (titulo, categoria, autor, editora, ano, valor, sinopse, imagem, destaque) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [titulo, categoria, autor, editora, ano, valor, sinopse, imagem, destaque],
        function (erro) {
            if (erro) {
                console.log(erro.message);
